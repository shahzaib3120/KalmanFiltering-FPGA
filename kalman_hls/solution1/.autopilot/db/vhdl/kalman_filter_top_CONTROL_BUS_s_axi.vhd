...
entity kalman_filter_top_CONTROL_BUS_s_axi is
generic (
...
end entity kalman_filter_top_CONTROL_BUS_s_axi;
...
architecture behave of kalman_filter_top_CONTROL_BUS_s_axi is
    ...
    constant ADDR_P_DATA_0 : INTEGER := 16#24#;
    constant ADDR_P_DATA_1 : INTEGER := 16#28#;
    constant ADDR_P_CTRL   : INTEGER := 16#2c#;
    constant ADDR_Z_BASE   : INTEGER := 16#10#;
    constant ADDR_Z_HIGH   : INTEGER := 16#17#;
    ...
    signal int_P               : UNSIGNED(63 downto 0) := (others => '0');
    -- memory signals
    signal int_z_address0      : UNSIGNED(0 downto 0);
    signal int_z_ce0           : STD_LOGIC;
    signal int_z_q0            : UNSIGNED(31 downto 0);
    signal int_z_address1      : UNSIGNED(0 downto 0);
    signal int_z_ce1           : STD_LOGIC;
    signal int_z_we1           : STD_LOGIC;
    signal int_z_be1           : UNSIGNED(3 downto 0);
    signal int_z_d1            : UNSIGNED(31 downto 0);
    signal int_z_q1            : UNSIGNED(31 downto 0);
    signal int_z_read          : STD_LOGIC;
    signal int_z_write         : STD_LOGIC;

    component kalman_filter_top_CONTROL_BUS_s_axi_ram is
        generic (
    ...
    end component kalman_filter_top_CONTROL_BUS_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_z
int_z : kalman_filter_top_CONTROL_BUS_s_axi_ram
generic map (
     MEM_STYLE => "auto",
     MEM_TYPE  => "2P",
     BYTES     => 4,
...
port map (
     clk0      => ACLK,
     address0  => int_z_address0,
     ce0       => int_z_ce0,
     we0       => (others=>'0'),
     d0        => (others=>'0'),
     q0        => int_z_q0,
     clk1      => ACLK,
     address1  => int_z_address1,
     ce1       => int_z_ce1,
     we1       => int_z_be1,
     d1        => int_z_d1,
     q1        => int_z_q1);
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_z_read = '0') else '0';
    RVALID    <= RVALID_t;
...
-- ----------------------- Memory logic ------------------
    -- z
    int_z_address0       <= UNSIGNED(z_address0);
    int_z_ce0            <= z_ce0;
    z_q0                 <= STD_LOGIC_VECTOR(RESIZE(int_z_q0, 32));
    int_z_address1       <= raddr(2 downto 2) when ar_hs = '1' else waddr(2 downto 2);
    int_z_ce1            <= '1' when ar_hs = '1' or (int_z_write = '1' and WVALID  = '1') else '0';
    int_z_we1            <= '1' when int_z_write = '1' and w_hs = '1' else '0';
    int_z_be1            <= UNSIGNED(WSTRB) when int_z_we1 = '1' else (others=>'0');
    int_z_d1             <= UNSIGNED(WDATA);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (ar_hs = '1' and raddr >= ADDR_Z_BASE and raddr <= ADDR_Z_HIGH) then
                    int_z_read <= '1';
                else
                    int_z_read <= '0';
                end if;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (aw_hs = '1' and UNSIGNED(AWADDR(ADDR_BITS-1 downto 0)) >= ADDR_Z_BASE and UNSIGNED(AWADDR(ADDR_BITS-1 downto 0)) <= ADDR_Z_HIGH) then
                    int_z_write <= '1';
                elsif (w_hs = '1') then
                    int_z_write <= '0';
                end if;
...
entity kalman_filter_top_CONTROL_BUS_s_axi_ram is
    generic (
        MEM_STYLE : STRING :="auto";
        MEM_TYPE  : STRING :="S2P";
        BYTES   : INTEGER :=4;
...
end entity kalman_filter_top_CONTROL_BUS_s_axi_ram;

architecture behave of kalman_filter_top_CONTROL_BUS_s_axi_ram is
