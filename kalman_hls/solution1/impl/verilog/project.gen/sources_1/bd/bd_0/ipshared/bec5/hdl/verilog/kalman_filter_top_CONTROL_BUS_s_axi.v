...
// ==============================================================
`timescale 1ns/1ps
module kalman_filter_top_CONTROL_BUS_s_axi
#(parameter
    ...
    input  wire [0:0]                    z_address0,
    input  wire                          z_ce0,
    output wire [31:0]                   z_q0,
    output wire [63:0]                   x,
    output wire [63:0]                   P,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_P = 'b0;
    // memory signals
    wire [0:0]                    int_z_address0;
    wire                          int_z_ce0;
    wire [31:0]                   int_z_q0;
    wire [0:0]                    int_z_address1;
    wire                          int_z_ce1;
    wire                          int_z_we1;
    wire [3:0]                    int_z_be1;
    wire [31:0]                   int_z_d1;
...
//------------------------Instantiation------------------
// int_z
kalman_filter_top_CONTROL_BUS_s_axi_ram #(
    .MEM_STYLE ( "auto" ),
    .MEM_TYPE  ( "2P" ),
    .BYTES     ( 4 ),
    .DEPTH     ( 2 )
...
assign RVALID  = (rstate == RDDATA) & !int_z_read;
...
//------------------------Memory logic-------------------
// z
assign int_z_address0 = z_address0;
assign int_z_ce0      = z_ce0;
assign z_q0           = int_z_q0;
assign int_z_address1 = ar_hs? raddr[2:2] : waddr[2:2];
assign int_z_ce1      = ar_hs | (int_z_write & WVALID);
assign int_z_we1      = int_z_write & w_hs;
assign int_z_be1      = int_z_we1 ? WSTRB : 'b0;
assign int_z_d1       = WDATA;
// int_z_read
always @(posedge ACLK) begin
    if (ARESET)
        int_z_read <= 1'b0;
    else if (ACLK_EN) begin
        if (ar_hs && raddr >= ADDR_Z_BASE && raddr <= ADDR_Z_HIGH)
            int_z_read <= 1'b1;
        else
            int_z_read <= 1'b0;
    end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (aw_hs && AWADDR[ADDR_BITS-1:0] >= ADDR_Z_BASE && AWADDR[ADDR_BITS-1:0] <= ADDR_Z_HIGH)
            int_z_write <= 1'b1;
        else if (w_hs)
            int_z_write <= 1'b0;
    end
...
module kalman_filter_top_CONTROL_BUS_s_axi_ram
#(parameter
    MEM_STYLE = "auto",
    MEM_TYPE  = "S2P",
    BYTES  = 4,
